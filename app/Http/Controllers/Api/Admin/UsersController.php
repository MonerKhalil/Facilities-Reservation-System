...
class UsersController extends Controller
{
    ...
    public function SearchUsersRule(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            $users = User::with("profile")->where("role",$request->role)
    ...
    private function GetCountUsersInMonth($role,$num){
    ...
    public function CountUsersInSystem(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $validate = Validator::make($request->all(),[
                "role"=>["nullable","string",Rule::in(["0","1"])]
            ]);
            if($validate->fails())
            {
                return \response()->json([
            ...
            $users = is_null(($request->role)) ? User::all()->where("role","!=","2")->count() : User::all()->where("role",$request->role)->count();
...
//            }
    ...
    public function ShowUsersAllRole(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $validate = Validator::make($request->all(),[
